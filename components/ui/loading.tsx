  ...
  return (
    <div className="fixed inset-0 bg-black flex items-center justify-center z-50">
      {/* DarkVeil-inspired animated background */}
        <motion.div
		 className="absolute top-1/4 left-1/4 w-96 h-96 bg-linear-to-r from-orange-600/30 to-red-600/20 rounded-full"
		...
		/>
        <motion.div
          className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-linear-to-r from-yellow-600/25 to-orange-500/25 rounded-full"
          initial={{ opacity: 0, scale: 0.9, filter: "blur(32px)" }}
          animate={{ 
        ...
        />
        <motion.div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-linear-to-r from-orange-500/30 to-amber-500/20 rounded-full"
          initial={{ opacity: 0, scale: 0.9, filter: "blur(32px)" }}
          animate={{ 
            opacity: [0.2, 0.4, 0.2],
            scale: [0.9, 1.1, 0.9],
        ...
        {/* Animated particles - Orange theme */}
          {particles.map((particle, i) => (
            <motion.div
              ...
              initial={{ opacity: 0.75, scale: 0.9, left: `${particle.left}%`, top: `${particle.top}%` }}
              animate={{ opacity: [0.5, 1, 0.5], scale: [0.9, 1.1, 0.9], left: `${particle.left}%`, top: `${particle.top}%` }}
              transition={{ duration: particle.duration, ease: "linear", repeat: Infinity }}
            />
          ))}
      ...
      <div className="relative z-10 text-center">
        ...
        <div className="mb-8">
          <div className="w-20 h-20 mx-auto mb-4 relative">
            ...
            <motion.div
              className="absolute inset-0 bg-linear-to-r from-orange-500 via-amber-500 to-yellow-500 rounded-full"
              initial={{ rotate: 0 }}
              animate={{ rotate: 360 }}
        ...
        <div className="mb-8">
          <div className="w-32 h-32 mx-auto relative">
            ...
            <motion.div className="absolute inset-0 border-4 border-transparent border-t-orange-500 border-r-amber-500 rounded-full"
              initial={{ rotate: 0 }}
              animate={{ rotate: 360 }}
              transition={{ duration: 1, ease: "linear", repeat: Infinity }}
            />
            <motion.div
              className="absolute inset-2 border-4 border-transparent border-b-yellow-500 border-l-orange-400 rounded-full"
              initial={{ rotate: 0 }}
              animate={{ rotate: 360 }}
              transition={{ duration: 1.5, ease: "linear", repeat: Infinity }}
            />
            <motion.div
              className="absolute inset-4 border-4 border-transparent border-t-amber-400 border-r-yellow-400 rounded-full"
              initial={{ rotate: 0 }}
              animate={{ rotate: 360 }}
        ...
        <div className="text-lg font-medium">
          <motion.span className="bg-linear-to-r from-orange-400 via-amber-400 to-yellow-400 bg-clip-text text-transparent "
            initial={{ opacity: 0.5 }}
